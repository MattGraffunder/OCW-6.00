...
def graphRemovedErr(points, kvals = [25, 50, 75, 100, 125, 150], cutoff = 0.1):
    ...
    for k in kvals:        
        ...
        #calc holdout error
        totalHoldoutError = holdoutError(clusters, partition[1])
        print totalHoldoutError
        
        kvalError[k] = (totalError, totalHoldoutError)
        clusterErrors.append(totalError)
        holdoutErrors.append(totalHoldoutError)
        
    #graph training error against holdout error
    pylab.title('Distance Squared Error of all Points')
    pylab.xlabel('Number of Clusters')
    pylab.ylabel('Error')
    pylab.plot(kvals, clusterErrors)
    pylab.plot(kvals, holdoutErrors)
    pylab.show()
        
    #graph the ratio between errors in separate graph
    ratios = []
    for i in xrange(len(kvals)):
        ratios.append(holdoutErrors[i] / clusterErrors[i])
    
    pylab.title('Ratio of Holdout Error to Training Set Error')
    pylab.xlabel('Number of Clusters')
    pylab.ylabel('Ratio of Error')
    pylab.plot(kvals, ratios)
    pylab.show()

def clusterError(clusters):
    error = 0.0;
...
def holdoutError(clusters, holdouts):
    error = 0.0
    ...
    for holdout in holdouts:        
        ...
        for cluster in clusters:
            distance = holdout.distance(cluster.getCentroid())
...
def GetMyCountyResuts(points):
    countyName = "MNHennepin"
    
    similarCounties = {}
    totalAppearances = 0
    for x in xrange(3):
        #Build Clusters
        clusters = kmeans(points, 50, 0.1, County)
        ...
        for cluster in clusters[0]:
            ...
            if cluster.contains(countyName):
                countyCluster = cluster
                break
        
        #Add each appearance
        for point in countyCluster.getPoints():
            countyName = point.getName()
            if countyName in similarCounties:
                similarCounties[countyName] += 1
            else:
                similarCounties[countyName] = 1
        
    #Print all counties is matching cluster
    print "Run " + str(x+1)
    for county in similarCounties:
        print 'County: ' + str(county) + " Appears: " + str(similarCounties[county])
        totalAppearances += similarCounties[county]
