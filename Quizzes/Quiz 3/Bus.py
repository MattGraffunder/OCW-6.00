...
class JobQueue(object):
    ...
    def depart(self):
        raise NotImplementedError('Must Override depart')
...
def simBus(bus, numStops = 6, loopLen = 1200, meanArrival = 90,
    ...
    assert loopLen%numStops == 0 #Check that number of stops divides evenly into loop

    #Build list of bus stop objects
    stops = []
    ...
    while time < simTime:
        ...
        for i in range(bus.getSpeed()):
            ...
            if (busLoc)%(loopLen/numStops) == 0:
                break #Stop moving bus if at a bus stop
        ...
        if lastArrival + nextJob.interArrival() <= time:
            ...
            for stop in stops:
                stop.arrive(nextJob) # looks like passenger arrives at all stops
            nextJob.queue(time)
            lastArrival = time
            #create new job for the next pass
            nextJob = Passenger(meanArrival, meanWork)
            
        #see if bus is at a stop
        if (busLoc)%(loopLen/numStops) == 0:
            #some passengers get off bus
            #bus unloads the number of people divided by the number of stops
            ...
            while stops[nextStop%numStops].length() > 0:
                try:
                    bus.enter() # Add passenger to bus
                except:
                    break
                p = stops[nextStop%numStops].depart() #remove passenger from queue
                totWait += time - p.queuedTime() #update the total wait time
                totPassengers += 1 #add to the total passengers processed
                time += p.work() #advance time, but not bus to show work needed from passenger
            try:
                aveWaitTimes.append(totWait/totPassengers) # Add average time for stop
    ...
    #Get then number of passengers that didn't get on buss
    leftWaiting = 0
    for stop in stops:
...
def test(capacities, speeds, numTrials):
    ...
    #Test all combinations of capacities and speed
    for cap in capacities:
        for speed in speeds:
            totWaitTimes = pylab.array([0.0]*500) #keep track of 1st 500 stops
            totLeftWaiting = 0.0
            
            #Run the combination for several trials
